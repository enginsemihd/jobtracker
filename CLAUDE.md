...
Bu, kişisel bir iş başvurusu takip uygulamasıdır. AI ile özgeçmiş/ön yazı uyarlama (Claude) ve çok kaynaklı iş arama (Jooble, Adzuna, Remotive, RemoteOK, Arbeitnow, Jobicy, Reed) içerir. Avrupa + Türkiye + UK pazarına odaklıdır. Çoklu kullanıcı destekli (JWT auth, her kullanıcı kendi profili/başvuruları).
...
DATABASE_URL, PORT (8080), JWT_SECRET, ANTHROPIC_API_KEY, JOOBLE_API_KEY, (opsiyonel) ADZUNA_APP_ID/ADZUNA_APP_KEY, (opsiyonel) REED_API_KEY, (opsiyonel) GOOGLE_CLIENT_ID + VITE_GOOGLE_CLIENT_ID (Google ile giriş)
